...
use crate::{Queue, process_event_source, process_client, State};
...
async fn generate_clients(num: u32, chat_state: State, clients_listner: &mut TcpListener) -> Vec<Peer> {
	let mut clients = vec![];
	for _ in 1..=num {
...
async fn follow() {
	...
	let mut clients = generate_clients(users_num, Arc::clone(&chat_state), &mut clients_listner).await;
	...
	assert_eq!(a, "a");
	assert_eq!(b, "b");
	assert!(chat_state.lock().await.users.get(&1).unwrap().followers.contains(&2));
}
...
async fn client() {
    ...
    assert_eq!(lines.next().await.unwrap().unwrap(), FOLLOW1_FROM1_TO2.split("\n").next().unwrap());
}
...
async fn event_source() {
	...
	process_event_source(Arc::clone(&chat_state), sequenced_queue, stream).await.unwrap();
